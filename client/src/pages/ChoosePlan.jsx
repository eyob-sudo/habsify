...
export default function ChoosePlan() {
  ...
  // ─── redirect state ───────────────────────────────────────────
  const [waitingForTrial, setWaitingForTrial] = useState(false)
  ...
  // ─── load payment methods + bank accounts when modal opens ────
  useEffect(() => {
    let alive = true
    if (payModalOpen) {
      Promise.all([
  ...
  // ─── polling query — only active after trial starts ───────────
  const { data: accessData } = useQuery({
    queryKey: ['accessStatus'],
    queryFn: () =>
      api
        .get('/subscriptions/me/access-status/', {
          headers: { 'Cache-Control': 'no-cache' },
        })
        .then((res) => res.data),
    enabled: waitingForTrial,
    refetchInterval: (data) => (data?.can_enter_app ? false : 300), // ✅ stops when granted
    retry: false,
    staleTime: 0,
  })

  // ─── watch access data — navigate exactly once ────────────────
  useEffect(() => {
    if (!accessData?.can_enter_app) return
    if (hasRedirected.current) return

    hasRedirected.current = true
    clearTimeout(fallbackTimer.current) // ✅ cancel the 15s fallback

    queryClient.setQueryData(['accessStatus'], accessData)
    setGlobalAccessStatus(accessData)
  ...
  // ─── called once on button press ─────────────────────────────
  const redirectToDashboard = () => {
    if (waitingForTrial) return // ✅ prevent double trigger

    toastId.current = toast.loading('Starting your trial...')
    setWaitingForTrial(true)

    // ✅ fallback — if backend never responds in 15s
  ...
  const handleStartTrial = async (planId) => {
    ...
    try {
      ...
      if (
        ...
        redirectToDashboard() // ✅ still try to redirect if they have access
  ...
  const handlePaySubmit = async (e) => {
    ...
    if (
      ...
      !payForm.bank_account ||
      !payForm.transaction_id.trim()
    ) {
    ...
    try {
      ...
      // ✅ Check once — if they have access already navigate, otherwise just stay
      ...
      if (updatedAccess?.can_enter_app) {
        queryClient.setQueryData(['accessStatus'], updatedAccess)
        setGlobalAccessStatus(updatedAccess)
    ...
    <div className="min-h-screen bg-gray-50 py-10 px-4 sm:px-6 lg:px-8">
      <div className="max-w-5xl mx-auto text-center">
        ...
        <p className="mt-3 text-lg text-gray-600">
          Select a plan that fits your business needs to continue.
        </p>
      </div>
      ...
      <div className="mt-10 max-w-md mx-auto grid gap-6 lg:grid-cols-3 lg:max-w-5xl">
        {plans.map((plan) => (
          <div
            key={plan.id}
            className="flex flex-col bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden transform hover:scale-[1.02] transition-transform duration-300"
          >
            <div className="p-6 bg-white">
              ...
              <p className="mt-3 text-sm text-gray-500">
                {plan.user_limit
                  ? `Up to ${plan.user_limit} users included.`
                  : 'Unlimited users'}
              </p>
            </div>
            ...
            <div className="flex-1 flex flex-col justify-between p-6 bg-gray-50/50 space-y-6">
                    ...
                    <div className="flex-shrink-0 mt-0.5">
                      {f.is_included ? (
                        <div className="w-4 h-4 rounded-full bg-green-100 flex items-center justify-center">
                          <i className="ri-check-line text-green-600 text-[10px]"></i>
                        </div>
                      ) : (
                        <div className="w-4 h-4 rounded-full bg-gray-200 flex items-center justify-center">
                          <i className="ri-close-line text-gray-500 text-[10px]"></i>
                        </div>
                      )}
                    </div>
                    <p
              ...
              <div className="space-y-3 pt-2">
                <button
                  ...
                  disabled={processing === plan.id || waitingForTrial}
                ...
                <button
                  ...
                  disabled={waitingForTrial}
                  className="w-full flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-lg text-white bg-primary hover:bg-primary/90 transition-colors shadow-sm disabled:opacity-50"
                >
                  Subscribe / Pay
                </button>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="mt-1.5 text-xs text-gray-500">
                    Please enter the exact transaction reference to verify your payment.
                  </p>
                </div>
              </div>
